...
def category_images_placeholder(instance, filename):
    path = os.path.join("Placeholders", "Category_Media", instance.category.slug, filename)
    return path
...
def review_image_placeholder_location(instance, filename):
    path = os.path.join(
        "Placeholders", "Reviews_Images", instance.review.user.username, instance.product.slug, filename
    )
    return path
